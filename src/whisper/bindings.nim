...
type
  WhisperContext* {.importc: "struct whisper_context", header: "whisper.h".} = object
  WhisperState* {.importc: "struct whisper_state", header: "whisper.h".} = object
  WhisperPos* = int32
  WhisperToken* = int32
  WhisperSeqId* = int32
  ...
  WhisperTokenData* {.importc: "struct whisper_token_data", header: "whisper.h", bycopy.} = object
  ...
  WhisperModelLoader* {.importc: "struct whisper_model_loader", header: "whisper.h", bycopy.} = object
    context* {.importc: "context".}: pointer
    read* {.importc: "read".}: proc (ctx: pointer; output: pointer; readSize: csize_t): csize_t {.
        cdecl.}
...
type
  WhisperSamplingStrategy* {.size: sizeof(cint).} = enum ## /////////////////////////////////////////////////////////////////////////
...
type
  WhisperNewSegmentCallback* = proc (ctx: ptr WhisperContext; state: ptr WhisperState;
                                  nNew: cint; userData: pointer) {.cdecl.} ##  Text segment callback
                                                                      ##  Called on every newly generated text segment
                                                                      ##  Use the whisper_full_...() functions to obtain the text segments

type
  WhisperProgressCallback* = proc (ctx: ptr WhisperContext; state: ptr WhisperState;
                                progress: cint; userData: pointer) {.cdecl.} ##  Progress callback

type
  WhisperEncoderBeginCallback* = proc (ctx: ptr WhisperContext;
                                    state: ptr WhisperState; userData: pointer): bool {.
...
type
  WhisperLogitsFilterCallback* = proc (ctx: ptr WhisperContext;
                                    ...
                                    logits: ptr cfloat; userData: pointer) {.cdecl.} ##  Logits filter callback
                                                                               ##  Can be used to modify the logits before sampling
                                                                               ##  If not NULL, called after applying temperature to logits

type
  INNER_C_STRUCT_whisper_0* {.importc: "no_name", header: "whisper.h", bycopy.} = object ##  Parameters for the whisper_full() function
  ...
  INNER_C_STRUCT_whisper_1* {.importc: "no_name", header: "whisper.h", bycopy.} = object
  ...
  WhisperFullParams* {.importc: "struct whisper_full_params", header: "whisper.h", bycopy.} = object
...
proc whisperInitFromFileWithParams*(pathModel: cstring;
    ...
    cdecl, importc: "whisper_init_from_file_with_params", dynlib: "libwhisper.so".}
  ##  Various functions for loading a ggml whisper model.
  ##  Allocate (almost) all memory needed for the model.
  ##  Return NULL on failure
proc whisperInitFromBufferWithParams*(buffer: pointer; bufferSize: csize_t;
                                     params: WhisperContextParams): ptr WhisperContext {.
    cdecl, importc: "whisper_init_from_buffer_with_params", dynlib: "libwhisper.so".}
...
proc whisperInitFromBufferWithParamsNoState*(buffer: pointer; bufferSize: csize_t;
    params: WhisperContextParams): ptr WhisperContext {.cdecl,
    importc: "whisper_init_from_buffer_with_params_no_state",
    dynlib: "libwhisper.so".}
proc whisperInitWithParamsNoState*(loader: ptr WhisperModelLoader;
                                  params: WhisperContextParams): ptr WhisperContext {.
    cdecl, importc: "whisper_init_with_params_no_state", dynlib: "libwhisper.so".}
...
proc whisperCtxInitOpenvinoEncoder*(ctx: ptr WhisperContext; modelPath: cstring;
                                   device: cstring; cacheDir: cstring): cint {.cdecl,
...
proc whisperPcmToMelPhaseVocoderWithState*(ctx: ptr WhisperContext;
    state: ptr WhisperState; samples: ptr cfloat; nSamples: cint; nThreads: cint): cint {.
    cdecl, importc: "whisper_pcm_to_mel_phase_vocoder_with_state",
    dynlib: "libwhisper.so".}
proc whisperSetMel*(ctx: ptr WhisperContext; data: ptr cfloat; nLen: cint; nMel: cint): cint {.
...
proc whisperSetMelWithState*(ctx: ptr WhisperContext; state: ptr WhisperState;
                            data: ptr cfloat; nLen: cint; nMel: cint): cint {.cdecl,
    importc: "whisper_set_mel_with_state", dynlib: "libwhisper.so".}
proc whisperEncode*(ctx: ptr WhisperContext; offset: cint; nThreads: cint): cint {.cdecl,
...
proc whisperDecodeWithState*(ctx: ptr WhisperContext; state: ptr WhisperState;
                            tokens: ptr WhisperToken; nTokens: cint; nPast: cint;
...
proc whisperLangAutoDetectWithState*(ctx: ptr WhisperContext;
                                    ...
                                    nThreads: cint; langProbs: ptr cfloat): cint {.
    cdecl, importc: "whisper_lang_auto_detect_with_state", dynlib: "libwhisper.so".}
proc whisperNLen*(ctx: ptr WhisperContext): cint {.cdecl, importc: "whisper_n_len",
    dynlib: "libwhisper.so".}
  ##  mel length
...
proc whisperTokenToStr*(ctx: ptr WhisperContext; token: WhisperToken): cstring {.cdecl,
...
proc whisperTokenLang*(ctx: ptr WhisperContext; langId: cint): WhisperToken {.cdecl,
...
proc whisperFullDefaultParamsByRef*(strategy: WhisperSamplingStrategy): ptr WhisperFullParams {.
    cdecl, importc: "whisper_full_default_params_by_ref", dynlib: "libwhisper.so".}
...
proc whisperFullParallel*(ctx: ptr WhisperContext; params: WhisperFullParams;
                         samples: ptr cfloat; nSamples: cint; nProcessors: cint): cint {.
...
proc whisperFullGetSegmentT0*(ctx: ptr WhisperContext; iSegment: cint): int64 {.cdecl,
    importc: "whisper_full_get_segment_t0", dynlib: "libwhisper.so".}
  ##  Get the start and end time of the specified segment
proc whisperFullGetSegmentT0FromState*(state: ptr WhisperState; iSegment: cint): int64 {.
    cdecl, importc: "whisper_full_get_segment_t0_from_state",
    dynlib: "libwhisper.so".}
proc whisperFullGetSegmentT1*(ctx: ptr WhisperContext; iSegment: cint): int64 {.cdecl,
    importc: "whisper_full_get_segment_t1", dynlib: "libwhisper.so".}
proc whisperFullGetSegmentT1FromState*(state: ptr WhisperState; iSegment: cint): int64 {.
    cdecl, importc: "whisper_full_get_segment_t1_from_state",
    dynlib: "libwhisper.so".}
proc whisperFullGetSegmentSpeakerTurnNext*(ctx: ptr WhisperContext; iSegment: cint): bool {.
...
proc whisperFullGetSegmentSpeakerTurnNextFromState*(state: ptr WhisperState;
    iSegment: cint): bool {.cdecl, importc: "whisper_full_get_segment_speaker_turn_next_from_state",
                         dynlib: "libwhisper.so".}
proc whisperFullGetSegmentText*(ctx: ptr WhisperContext; iSegment: cint): cstring {.
    cdecl, importc: "whisper_full_get_segment_text", dynlib: "libwhisper.so".}
  ##  Get the text of the specified segment
proc whisperFullGetSegmentTextFromState*(state: ptr WhisperState; iSegment: cint): cstring {.
...
proc whisperFullNTokensFromState*(state: ptr WhisperState; iSegment: cint): cint {.
    cdecl, importc: "whisper_full_n_tokens_from_state", dynlib: "libwhisper.so".}
proc whisperFullGetTokenText*(ctx: ptr WhisperContext; iSegment: cint; iToken: cint): cstring {.
...
proc whisperFullGetTokenId*(ctx: ptr WhisperContext; iSegment: cint; iToken: cint): WhisperToken {.
...
proc whisperFullGetTokenData*(ctx: ptr WhisperContext; iSegment: cint; iToken: cint): WhisperTokenData {.
...
proc whisperFullGetTokenP*(ctx: ptr WhisperContext; iSegment: cint; iToken: cint): cfloat {.
